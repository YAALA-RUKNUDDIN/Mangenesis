...
export default function ProductionForecast() {
  ...
  return (
    <PageLayout
      ...
      subtitle={`Predicting potential production shortfalls up to 7 days in advance for ${activeMineData.name} before they impact processing plant feed rate.`}
    >
      ...
      <div className="mb-6 rounded-2xl border border-[#262F3D] bg-[#131720]/90 p-4 shadow-card">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-3 mb-3 border-b border-[#262F3D]">
          ...
          <span className="text-[11px] font-mono text-slate-400">
            Simulate extraction bottlenecks to test LightGBM 7-day forecast response
          </span>
        </div>
      ...
      <div className="rounded-2xl border border-[#262F3D] bg-[#131720]/85 text-slate-100 shadow-card backdrop-blur-xl p-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 mb-5">
          ...
          <div className="flex items-center gap-4 text-xs">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-slate-400 text-[11px] font-mono">{activeMineData.capacity_tpd ? `${(activeMineData.capacity_tpd/1000).toFixed(0)}k` : '10k'} Target</span>
            </div>
            <div className="flex items-center gap-1.5">
        ...
        {/* Subtitle Divider */}
        <div className="mt-4 pt-3.5 border-t border-[#262F3D] flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs text-slate-400">
          <div className="flex items-center gap-2 text-[11px]">
            <span className="text-sky-400 font-medium">Aug 4 &ndash; Aug 17 (Actual Extraction)</span>
            <span className="text-slate-600">&rarr;</span>
            <span className="text-blue-400 font-bold">Aug 19 &ndash; Aug 25 (AI LightGBM Forecast)</span>
          </div>
          {scenarioData.expectedGap > 0 ? (
            <span className="text-[11px] text-rose-400 font-bold flex items-center gap-1.5 bg-rose-500/10 border border-rose-500/20 px-2.5 py-0.5 rounded-full">
              ...
              Projected Deficit: -{scenarioData.expectedGap.toLocaleString()} T (Days +4 to +6)
            </span>
          ) : (
            <span className="text-[11px] text-emerald-400 font-bold flex items-center gap-1.5 bg-emerald-500/10 border border-emerald-500/20 px-2.5 py-0.5 rounded-full">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-400" />
              Target Maintained: 100% Nominal Feed
            </span>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-5">
        <div className="rounded-2xl border border-emerald-500/30 bg-[#131720]/80 p-5 flex items-center justify-between shadow-card">
          <div>
        ...
        <div className="rounded-2xl border border-amber-500/30 bg-[#131720]/80 p-5 flex items-center justify-between shadow-card">
          <div>
        ...
        <div className="rounded-2xl border border-rose-500/30 bg-[#131720]/80 p-5 flex items-center justify-between shadow-card">
          <div>
            ...
            <div className="text-xs text-slate-400 mt-0.5">
              {scenarioData.expectedGap > 0 ? `Projected -${scenarioData.expectedGap.toLocaleString()} T deficit` : 'Normal extraction'}
            </div>
          </div>
          <StatusBadge level={scenarioData.riskLevel} size="md" />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 mt-5">
        ...
        <div className="lg:col-span-7 space-y-5">
          <div className="rounded-2xl border border-[#262F3D] bg-[#131720]/85 p-6 shadow-card">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
              <div className="p-4 bg-[#0B0D12] rounded-xl border border-[#262F3D]">
                ...
                <div className="text-[10px] text-slate-400 mt-0.5 font-mono">
                  {scenarioData.expectedGap > 0 ? 'Below planned target' : 'On target'}
                </div>
              </div>
        ...
        <div className="lg:col-span-5">
          <div className="rounded-2xl border border-[#262F3D] bg-[#131720]/85 p-6 shadow-card h-full flex flex-col justify-between">
            <div>
              <div className="flex items-center gap-2 mb-4 pb-3 border-b border-[#262F3D]">
              ...
              <div className="space-y-4 relative before:absolute before:left-[9px] before:top-2 before:bottom-2 before:w-px before:bg-[#262F3D]">
            ...
            <div className="mt-5 pt-3.5 border-t border-[#262F3D] text-[11px] text-slate-400 flex items-center gap-2 font-mono">
