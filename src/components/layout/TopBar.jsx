...
export default function TopBar() {
  ...
  return (
    <header className="h-16 min-h-[64px] bg-[#0A0E17]/85 backdrop-blur-xl border-b border-slate-800/80 flex items-center justify-between px-6 z-[2000] relative">
      {/* Left: Interactive MOIL Mine Selector Dropdown */}
      <div className="relative z-[2100]">
        <button
          onClick={() => setMineDropdownOpen(!mineDropdownOpen)}
          className="flex items-center gap-3 px-3.5 py-2 rounded-xl bg-[#0E131F] hover:bg-[#141C2E] border border-slate-800 text-xs text-slate-100 transition-all duration-200 shadow-card cursor-pointer group"
        >
          <div className="w-6 h-6 rounded-lg bg-blue-500/10 border border-blue-500/20 flex items-center justify-center text-blue-400 group-hover:scale-105 transition-transform">
            <Building2 size={13} />
          </div>

          <div className="text-left">
            <div className="flex items-center gap-1.5">
              <span className="font-semibold text-slate-100 tracking-tight">{activeMineData.name}</span>
              {activeMineData.pilot && (
                <span className="text-[9px] font-mono font-bold bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-1.5 py-0.2 rounded shadow-sm">
                  PILOT
                </span>
              )}
            </div>
            <div className="text-[10px] text-slate-400 font-mono">
              {activeMineData.district}, {activeMineData.state} &bull; {activeMineData.center ? `${activeMineData.center[0]}°N, ${activeMineData.center[1]}°E` : `${activeMineData.lat}°N, ${activeMineData.lon}°E`}
            </div>
          </div>

          <ChevronDown size={14} className="text-slate-400 ml-1 group-hover:text-slate-200 transition-colors" />
        </button>
          ...
          {mineDropdownOpen && (
            <>
              ...
              <motion.div
                ...
                className="absolute left-0 mt-2 w-96 bg-[#0E131F] border border-slate-800 rounded-2xl py-2.5 shadow-popover z-[2200] overflow-hidden"
              >
      ...
      <div className="flex items-center gap-3">
        {/* Date Display */}
        <div className="hidden sm:flex items-center gap-2 text-xs text-slate-300 font-mono px-3 py-1.5 rounded-xl bg-[#0E131F] border border-slate-800 shadow-card">
          <Calendar size={13} className="text-slate-400" />
          <span>18 Aug 2026</span>
        </div>

        {/* Space Data Active Badge */}
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs font-semibold shadow-glow-emerald">
        ...
        <div className="relative z-[2100]">
          <button
            ...
            className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-[#0E131F] hover:bg-[#141C2E] border border-slate-800 text-slate-200 text-xs font-mono transition-all cursor-pointer shadow-card"
            ...
            {supabaseModalOpen && (
              <>
                ...
                <motion.div
                  ...
                  exit={{ opacity: 0, y: 6, scale: 0.98 }}
                  transition={{ duration: 0.15 }}
                  className="absolute right-0 mt-2 w-80 sm:w-96 bg-[#0E131F] border border-slate-800 rounded-2xl p-5 shadow-popover z-[2200]"
                >
                  ...
                  <div className="py-3 space-y-3">
                    ...
                    >
                      <Zap size={14} />
                      {isSeeding ? 'Seeding Supabase DB...' : 'Sync & Seed MOIL Dataset to Supabase'}
                    </button>
        ...
        {/* Live Swagger API Docs Link */}
        <a
          href="http://localhost:8000/docs"
          target="_blank"
          rel="noreferrer"
          className="hidden md:flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-[#0E131F] hover:bg-[#141C2E] border border-slate-800 text-slate-300 hover:text-white text-xs font-mono transition-colors shadow-card"
        ...
        <div className="flex items-center gap-2 pl-3 border-l border-slate-800">
          <div className="w-7 h-7 rounded-xl bg-gradient-to-tr from-blue-600 to-indigo-600 flex items-center justify-center text-[10px] font-bold text-white shadow-glow-blue">
            MC
          </div>
